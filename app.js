...
const submitCreateAssignments = () => {
  ...
  if (currentSyncMode === 'firebase' && dbRef) {
    const updates = {};
    updates[`dailyAssignments/${activeCreateDate}`] = dailyAssignments[activeCreateDate];
    updates[`dailyLogs/${activeCreateDate}`] = dailyLogs[activeCreateDate];
    if (isOverwrite && existingTasks.length > 0) {
    ...
    dbRef.update(updates)
      ...
      .catch(err => {
        console.error("[Firebase] 과제 생성 업로드 실패:", err);
      });
  } else {
    saveData();
  }
...
const deleteCurrentDayAssignments = () => {
  if (confirm(`⚠️ 정말로 ${selectedRecordDate}의 과제판 데이터를 삭제하시겠습니까?\n모든 학생의 당일 완료 기록과 점수가 삭제 및 차감됩니다.`)) {
    ...
    if (currentSyncMode === 'firebase' && dbRef) {
      const updates = {};
      updates[`dailyAssignments/${selectedRecordDate}`] = null;
      updates[`dailyLogs/${selectedRecordDate}`] = null;
      updates[`students`] = students;
      updates[`pointHistory`] = pointHistory;
      dbRef.update(updates)
        .then(() => {
        ...
        .catch(err => {
          console.error("[Firebase] 과제 삭제 업로드 실패:", err);
        });
    } else {
      saveData();
    }
...
const deleteDashboardAssignments = () => {
  ...
  if (confirm(`⚠️ 정말로 ${dateKey}의 모든 과제판 데이터를 삭제하시겠습니까?\n모든 학생의 당일 완료 기록과 점수가 복원 및 차감됩니다.`)) {
    ...
    if (currentSyncMode === 'firebase' && dbRef) {
      const updates = {};
      updates[`dailyAssignments/${dateKey}`] = null;
      updates[`dailyLogs/${dateKey}`] = null;
      updates[`students`] = students;
      updates[`pointHistory`] = pointHistory;
      dbRef.update(updates)
        .then(() => {
...
const importClassroomData = (event) => {
  ...
  reader.onload = (e) => {
    try {
      ...
      // localStorage 데이터 복원
      localStorage.setItem('restore_pending_upload', 'true');
      ...
      localStorage.setItem('dailyAssignments', JSON.stringify(cleanAssignments(data.dailyAssignments || {})));
      ...
      if (currentSyncMode === 'firebase' && dbRef) {
        ...
        config = data.config || {};
        dailyAssignments = cleanAssignments(data.dailyAssignments || {});
        pendingRequests = data.pendingRequests || {};
        absentLogs = data.absentLogs || {};
...
// @TEACHER_ONLY_END
  ...
  // @TEACHER_ONLY_START
  const pendingLogin = localStorage.getItem('teacher_login_pending');
  if (pendingLogin) {
    localStorage.removeItem('teacher_login_pending');
    if (pendingLogin === 'google') {
      loginAsTeacher();
    } else if (pendingLogin === 'local') {
      loginLocalBypass();
    }
  }
  const pendingSettings = localStorage.getItem('teacher_settings_pending');
  if (pendingSettings) {
    localStorage.removeItem('teacher_settings_pending');
    toggleLandingSettings();
  }
  // @TEACHER_ONLY_END
...
const restoreFromDailyBackup = () => {
  ...
  dailyAssignments = cleanAssignments(backup.dailyAssignments || dailyAssignments);
